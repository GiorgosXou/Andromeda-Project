...
public final class MultiplicationExpression extends ArithmeticExpression {
   ...
   * @param leftOperand  operand to multiply
   * @param rightOperand  operand to multiply by
   */
  public MultiplicationExpression(long position, Expression leftOperand, Expression rightOperand) {
  ...
  protected Expression fold(Compiler compiler, FunctionSymbol currentFunction) {
    if (leftOperand instanceof ConstantNumberExpression &&
      ...
      return new ConstantNumberExpression(getPosition(),
          ((ConstantNumberExpression) leftOperand).value.multiply(
